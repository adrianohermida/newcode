type ProcessosModuleProps = {
  data: any[];
  onSelect: (processo: any) => void;
};

export const ProcessosModule = ({ data, onSelect }: ProcessosModuleProps) => {
          ...
          <div className="flex justify-between items-center">
            <div>
              <p className="text-xs text-white/40">
                Processo #{processo.id} • {processo.cliente_email}
              </p>
              <h3 className="font-bold">{processo.assunto}</h3>
              <p className="text-xs text-white/50">
                Status: {processo.status} • Prioridade: {processo.prioridade}
              </p>
            </div>
