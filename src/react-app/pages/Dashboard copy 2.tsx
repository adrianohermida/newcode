/**
 * @description Painel Administrativo – Hermida Maia Advocacia
 * Dashboard modular, desacoplado e escalável
 */
...
export const Dashboard: React.FC = () => {
      ...
      try {
        switch (activeTab) {
          case 'crm': {
            ...
            setCrmData(await res.json());
            break;
          }

          case 'processos': {
            const res = await fetch('/api/admin/processos');
            setProcessosData(await res.json());
            break;
          }

          case 'faturas': {
            const res = await fetch('/api/admin/faturas');
            setFaturasData(await res.json());
            break;
          }

          case 'tickets': {
            const res = await fetch('/api/tickets');
            setTicketsData(await res.json());
            break;
          }
          ...
          case 'agenda': {
            ...
            const parsed = raw.map((a: any) => ({
              ...a,
              form_data:
                typeof a.form_data === 'string'
                  ? JSON.parse(a.form_data)
                  : a.form_data,
            }));
          ...
          case 'documentos': {
            ...
            setDocumentsData(await res.json());
            break;
          }

          case 'config': {
            const res = await fetch('/api/admin/config/status');
            setIntegrationsStatus(await res.json());
            break;
          }
        }
      } catch (err) {
  ...
  const filteredProcessos = useMemo(
    () =>
      processosData.filter(p =>
        Object.values(p).some(v =>
          String(v).toLowerCase().includes(searchTerm.toLowerCase())
        )
      ),
    [processosData, searchTerm]
  );
  ...
  <div className="space-y-8">
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {[
        ...
        { label: 'Faturamento Mês', value: 'R$ 45k', icon: TrendingUp, color: 'text-green-399' },
